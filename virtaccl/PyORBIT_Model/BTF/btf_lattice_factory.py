...
class PyORBIT_Lattice_Factory:
    ...
    def getLinacAccLatticeFromDA(self, names, acc_da):
        ...
        # -----let's organize lattice by input order.
        new_accSeq_da_arr = []
        for name in names:
            for seq in accSeq_da_arr:
                if seq.getName() == name:
                    new_accSeq_da_arr.append(seq)
        accSeq_da_arr = new_accSeq_da_arr
        # ----make linac lattice
        linacAccLattice = LinacAccLattice(acc_da.getName())

        # There are the following possible types of elements in the linac tree:
        # QUAD - quadrupole
        # RFGAP - RF Gap
        # DCH - horizontal dipole corrector
        # DCV - vertical dipole corrector
        # Marker - anything else with the length equals to 0
        # Before putting everything into the linacAccLattice we will create sequences
        # with all nodes.
        # ----------------------------------------------------------------------
        ...
        for seq_da in accSeq_da_arr:
            ...
            for node_da in node_da_arr:
                ...
                if node_type == "QUAD":
                    ...
                    # Add Correctors to specific quadrupoles in lattice
