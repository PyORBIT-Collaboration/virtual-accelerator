...
if __name__ == '__main__':
    ...
    parser.add_argument('--file', '-f', default='../../EPICS/va_config.json', type=str,
    ...
    lattice_file = '../../EPICS/sns_linac.xml'
    ...
    bunch_in = Bunch()
    bunch_in.readBunch('../../EPICS/SCL_in.dat')
    dx = 0.0001
    dxp = -0.00001
    dy = -0.000001
    dyp = 0.00
    for n in range(bunch_in.getSizeGlobal()):
        ...
        bunch_in.x(n, x + dx)
        xp = bunch_in.xp(n)
        bunch_in.xp(n, xp + dxp)
        y = bunch_in.y(n)
        bunch_in.y(n, y + dy)
        yp = bunch_in.yp(n)
        bunch_in.yp(n, yp + dyp)
        if n + 1 > 1000:
    ...
    with open('../../EPICS/va_offsets.json', "r") as json_file:
